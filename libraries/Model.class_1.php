...
class Model{
    ...
    public static function saveGeneralForm(&$sDialog = null){
        ...
        if(empty($aErrors)){
            ...
            $_SESSION['reservation']['reservation-date'] = db_str(http_post('reservation-date',null));
            
            $aData['point'] = db_str(http_post('access-point',null));
            $_SESSION['reservation']['access-point'] = db_str(http_post('access-point',null));
            
            $aData['hour'] = db_str(http_post('schedule',null));
            $_SESSION['reservation']['schedule'] = db_str(http_post('schedule',null));
            
            $aData['members-amount'] = db_str(http_post('extra-members-number',null) + 1);
            $_SESSION['reservation']['extra-members-number'] = db_str(http_post('extra-members-number',null));
            
            // Personal info
            $aData['responsible']['name'] = db_str(http_post('name',null));
            $_SESSION['reservation']['name'] = db_str(http_post('extra-members-number',null));
            
            $aData['responsible']['surname'] = db_str(http_post('surname',null));
            $_SESSION['reservation']['name'] = db_str(http_post('surname',null));
            
            $aData['responsible']['birthdate'] = db_str(http_post('birthdate',null));
            $_SESSION['reservation']['birthdate'] = db_str(http_post('birthdate',null));
            
            $aData['responsible']['dni'] = db_str(http_post('dni',null));
            $_SESSION['reservation']['dni'] = db_str(http_post('dni',null));
            $_SESSION['reservation']['select-document'] = db_str(http_post('select-document',null));
            
            $aData['responsible']['phonecell'] = db_str(http_post('mobile',null));
            $_SESSION['reservation']['mobile'] = db_str(http_post('mobile',null));
            
            $aData['responsible']['phonefixed'] = db_str(http_post('phone',null));
            $_SESSION['reservation']['phone'] = db_str(http_post('phone',null));
            
            $aData['responsible']['email'] = db_str(http_post('email',null));
            $_SESSION['reservation']['email'] = db_str(http_post('email',null));
            $_SESSION['reservation']['emailValidation'] = db_str(http_post('emailValidation',null));
            ...
            for($iNumMember = 1;$iNumMember<=http_post('extra-members-number',null);$iNumMember++){
                ...
                $_SESSION['reservation']['under-age-em'][$iNumMember] = isset($_POST['under-age-em'][$iNumMember]) ? db_str($_POST['under-age-em'][$iNumMember]) : null;
                
                $aMember['name'] = isset($_POST['name-em'][$iNumMember]) ? db_str($_POST['name-em'][$iNumMember]): null;
                $_SESSION['reservation']['name-em'][$iNumMember] = isset($_POST['name-em'][$iNumMember]) ? db_str($_POST['name-em'][$iNumMember]): null;
                
                $aMember['surname'] = isset($_POST['surname-em'][$iNumMember]) ? db_str($_POST['surname-em'][$iNumMember]) : null;
                $_SESSION['reservation']['surname-em'][$iNumMember] = isset($_POST['surname-em'][$iNumMember]) ? db_str($_POST['surname-em'][$iNumMember]) : null;
                
                $aMember['birthday'] = isset($_POST['birthdate-em'][$iNumMember]) ? db_str($_POST['birthdate-em'][$iNumMember]): null;
                $_SESSION['reservation']['birthdate-em'][$iNumMember] = isset($_POST['birthdate-em'][$iNumMember]) ? db_str($_POST['birthdate-em'][$iNumMember]): null;
                
                $aMember['dni'] = isset($_POST['dni-em'][$iNumMember]) ? db_str($_POST['dni-em'][$iNumMember]):null ;
                $_SESSION['reservation']['dni-em'][$iNumMember] = isset($_POST['dni-em'][$iNumMember]) ? db_str($_POST['dni-em'][$iNumMember]):null ;
                $_SESSION['reservation']['select-document-em'][$iNumMember] = isset($_POST['select-document-em'][$iNumMember]) ? db_str($_POST['select-document-em'][$iNumMember]):null ;
